...
bool testSphereOBox(AbstractModel::SBB sbb, AbstractModel::OBB obb)
{
	...
	if (sbb.c[0] >= aabb.mins[0] && sbb.c[0] <= aabb.maxs[0] && sbb.c[1] >= aabb.mins[1] && sbb.c[1] <= aabb.maxs[1] && sbb.c[2] >= aabb.mins[2] && sbb.c[2] <= aabb.maxs[2])
		return true;
	for (int i = 0; i < 3; i++)
	{
...
bool testRayOBB(glm::vec3 o, glm::vec3 t, AbstractModel::OBB obb)
{
	...
	glm::vec3 cABB = qinv * obb.c;
	glm::vec3 mins = cABB - obb.e;
	glm::vec3 maxs = cABB + obb.e;
